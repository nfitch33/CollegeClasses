...
let fail_programs =
  ...
  |> List.map (fun s -> read_file @@ "../../../tests/fail/" ^ s)
...
let pass_expected_values : value list =
  Result.get_ok @@
    mapM (fun nm ->
        let* s = sexp_of_string @@ read_file @@ "../../../tests/pass/" ^ nm ^ ".expected" in
        parse_value s) pass_names

(** Run test programs that are expected to produce either parser or
   interpreter errors. *)
let () = add_test "run_fail" @@
           fun _ ->
           List.iter (fun s ->
...
let () = add_test "run_pass" @@
           fun _ ->
           List.iter (fun (s, v) ->
               (check @@ res value) s (Ok v) (run s))
...
let () =
  Alcotest.run "PA6"
  [ ("test", !tests)
  ; ("qcheck", !qcheck_tests @ Llm_test.llm_pa6_tests)
  ]
