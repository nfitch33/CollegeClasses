# CS3200 - Organization of Programming Languages: Introduction
...
https://cacmb4.acm.org/blogs/blog-cacm/176450-python-is-now-the-most-popular-introductory-teaching-language-at-top-us-universities/fulltext
...
{
  enum button res = NOTHING;
  for (size_t i=0; i<n; i++){
    enum button x = events[i];
    if      (res==LIKE    && x==DISLIKE)  res = DISLIKE;
    else if (res==NOTHING && x==DISLIKE)  res = DISLIKE;
    else if (res==DISLIKE && x==DISLIKE)  res = NOTHING;
    else if (res==LIKE    && x==LIKE   )  res = NOTHING;
    else if (res==NOTHING && x==LIKE   )  res = LIKE;
    else if (res==DISLIKE && x==LIKE   )  res = LIKE;
  }
  return res;
}
```
