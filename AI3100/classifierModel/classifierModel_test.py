...
def train_model(model, criterion, optimizer, train_loader, val_loader, start_epoch, num_epochs, checkpoint_path, class_to_idx):
    ...
    for epoch in range(start_epoch, start_epoch + num_epochs):
        ...
        val_loss, val_acc, val_labels, val_preds = evaluate_model(model, criterion, val_loader)
...
# Test the model and generate confusion matrix scatterplot
def test_model(model, test_loader, criterion):
    loss, acc, labels, preds = evaluate_model(model, criterion, test_loader)

    precision = precision_score(labels, preds, average='macro', zero_division=0)
    recall = recall_score(labels, preds, average='macro', zero_division=0)
    f1 = f1_score(labels, preds, average='macro', zero_division=0)
    cm = confusion_matrix(labels, preds)
    report = classification_report(labels, preds, target_names=test_dataset.classes, zero_division=0)
    ...
    # Create a scatterplot of confusion matrix
    plt.figure(figsize=(10, 10))
    for i in range(cm.shape[0]):
        for j in range(cm.shape[1]):
            if cm[i, j] > 0:
                plt.scatter(j, i, s=cm[i, j]*10, c='blue', alpha=0.6)  # size proportional to count
                plt.text(j, i, str(cm[i, j]), color='red', ha='center', va='center', fontsize=8)
    ...
    plt.ylabel("Actual")
    plt.title("Confusion Matrix Scatterplot")
    plt.tight_layout()
    plt.savefig("confusion_scatter.png", dpi=300)
    print("[INFO] Confusion matrix scatterplot saved as 'confusion_scatter.png'")

# Main
if __name__ == "__main__":
    model_path = "fish_classifier3.pth"
